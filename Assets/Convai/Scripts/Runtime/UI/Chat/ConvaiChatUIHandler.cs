...
namespace Convai.Scripts.Runtime.UI
{
    ...
    public class ConvaiChatUIHandler : MonoBehaviour
    {
        ...
        private void Awake()
        {
            File.WriteAllText(@".\playerTranscript.txt",  "");
            if (Instance != null)
            {
        ...
        {
            //file writing for transcript
            File.AppendAllText(@".\playerTranscript.txt",  charName + ": " + text + Environment.NewLine);
        ...
        {
            //Parses player text to update top-right HUD
            string textParse = text.ToLower();
            string[] nameArr = {"name"};
            string[] idArr = {"ID", "id", "identification"};
            string[] dobArr = {"birth", "date", "DOB", "date of birth", "Date"};
            string[] purposeArr = {"purpose"};
            string[] checkUnderstandingArr = {"understand", "clear"};
            string[] comfortArr = {"comfortable", "comfort", "feeling","feel"};
          
            if(nameArr.Any(textParse.Contains))
            {
                popUpText.text = "! - Recorded Patient's Name";
            }
            else if(idArr.Any(textParse.Contains))
            {
                popUpText.text = "! - Recorded Patient's ID";
            }
            else if(dobArr.Any(textParse.Contains))
            {
                popUpText.text = "! - Recorded Patient's D.O.B";
            }
             else if(purposeArr.Any(textParse.Contains))
            {
                popUpText.text = "! - Explained Purpose of Interaction ";
            }
             else if(checkUnderstandingArr.Any(textParse.Contains))
            {
                popUpText.text = "! - Recorded Patient's Understanding";
            }
            else if(comfortArr.Any(textParse.Contains))
            {
                popUpText.text = "! - Checked Patient's Comfort";
            }
            ...
            //File writing for transcript
            string exportText = "Player: " + text + Environment.NewLine;
            File.AppendAllText(@".\playerTranscript.txt", exportText);
            string[] lines = File.ReadAllLines(@".\playerTranscript.txt");
            File.WriteAllLines(@".\playerTranscript.txt", lines.Distinct().ToArray());
